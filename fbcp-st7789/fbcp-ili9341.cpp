...
*/
    ...
    if (!CS_BIT)
        ...
        yStartShift = actualDisplayHeight;
#endif

    Span *head = 0;

#if defined(ALL_TASKS_SHOULD_DMA) && defined(UPDATE_FRAMES_WITHOUT_DIFFING)
    NoDiffChangedRectangle(head, yStartShift);
    ...
    if (framebufferHasNewChangedPixels || prevFrameWasInterlacedUpdate)
    {
      ...
      if (gpuFrameWidth % 4 == 0 && gpuFramebufferScanlineStrideBytes % 8 == 0)
        ...
        DiffFramebuffersToScanlineSpansFastAndCoarse4Wide(framebuffer[0], framebuffer[1], interlacedUpdate, frameParity, yStartShift, head);

      else
#endif

        DiffFramebuffersToScanlineSpansExact(framebuffer[0], framebuffer[1], interlacedUpdate, frameParity, yStartShift, head); // If disabled, or framebuffer width is not compatible, use the exact method
    ...
    for(Span *i = head; i; i = i->next)
    {
      ...
      uint16_t *scanline = framebuffer[0] + (i->y + yStartShift) * (gpuFramebufferScanlineStrideBytes>>1);
      uint16_t *prevScanline = framebuffer[1] + (i->y + yStartShift) * (gpuFramebufferScanlineStrideBytes>>1);
