...
#define PIN_MOUSECAM_RESET             11 // 11 top, 5 bottom
#define PIN_SS                         10 // 10 top, 4 bottom
...
void loop() {
  if(SERIAL.available()){
    byte a = SERIAL.read();
    if(a == 'h'){
      mousecam_frame_capture(frame);
      SERIAL.write(frame, ADNS3080_PIXELS_X*ADNS3080_PIXELS_Y);
    }
  }
}
