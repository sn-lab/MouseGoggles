...
export var rotation_type = 3 #1=yaw, 2=pitch or 3=roll
export var reward_dur = 0.05 #duration to open liquid reward valve)

#eye parameters
export var inter_eye_distance = 0.01
export var head_radius = 0.02 #must be large enough to keep eye cameras from getting too close to walls
...
var head_x = 0
var head_z = 0
var head_y = head_radius
var head_yaw_mod = 0
var head_pitch_mod = 0
var head_roll_mod = 0
...
func _ready():	
	...
	lefteye.rotation_degrees.x = head_pitch_mod+eye_pitch
	lefteye.rotation_degrees.y = -head_yaw_mod+eye_yaw
	lefteye.rotation_degrees.z = head_roll_mod+head_roll
	righteye.rotation_degrees.x = head_pitch_mod+eye_pitch
	righteye.rotation_degrees.y = -head_yaw_mod-eye_yaw
	righteye.rotation_degrees.z = head_roll_mod+head_roll
	...
	lefteye.translation.x = head_x - (0.5*inter_eye_distance*cos(deg2rad(head_yaw+head_yaw_mod))*cos(deg2rad(head_roll+head_roll_mod)))
	lefteye.translation.y = head_y - (0.5*inter_eye_distance*sin(deg2rad(head_roll+head_roll_mod)))
	lefteye.translation.z = head_z - (0.5*inter_eye_distance*sin(deg2rad(head_yaw+head_yaw_mod))*cos(deg2rad(head_roll+head_roll_mod)))
	righteye.translation.x = head_x + (0.5*inter_eye_distance*cos(deg2rad(head_yaw+head_yaw_mod))*cos(deg2rad(head_roll+head_roll_mod)))
	righteye.translation.y = head_y + (0.5*inter_eye_distance*sin(deg2rad(head_roll+head_roll_mod)))
	righteye.translation.z = head_z + (0.5*inter_eye_distance*sin(deg2rad(head_yaw+head_yaw_mod))*cos(deg2rad(head_roll+head_roll_mod)))
...
func _process(delta):
	...
	head_z += mouse_gain*(thrust_gain*head_thrust*cos(deg2rad(head_yaw+head_yaw_mod)) + slip_gain*head_slip*sin(deg2rad(head_yaw+head_yaw_mod)))
	if (head_z>0.05):
		head_z -= 0.1
	if (head_z<-0.05):
		head_z += 0.1
...
#	fpslabel.text = str(fps) + " FPS" 
	if rotation_type==1:
		fpslabel.text = str(head_yaw+head_yaw_mod)
	if rotation_type==2:
		fpslabel.text = str(eye_pitch+head_pitch_mod)
	if rotation_type==3:
		fpslabel.text = str(head_roll+head_roll_mod)
...
func _input(ev):
	...
	if ev is InputEventMouseMotion:
		if rotation_type==1:
			head_yaw_mod = 2*ev.relative.x #rotation sensor sends angle in 2-deg increments
		if rotation_type==2:
			head_pitch_mod = 2*ev.relative.x
		if rotation_type==3:
			head_roll_mod = 2*ev.relative.x 
