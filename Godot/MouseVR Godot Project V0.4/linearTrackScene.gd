...
export var track_num = 1 #1 = track A (grey), 2 = track B (green)

#track variables
export var track1_xpos = 0; #center x of linear track
export var track1_reward_loc = -0.2 #location of reward (start wall = -0.5, end wall = 0.5)
export var track2_xpos = 0.5
export var track2_reward_loc = 0.3
...
export var track_xpos = 0;
export var track_reward_loc = 0;
...
#	timestamp = Time.get_time_string_from_system() # OS.get_datetime deprecated in Godot 4?
	...
	if track_num==1:
		...
		track_reward_loc = track1_reward_loc;
	if track_num==2:
		track_xpos = track2_xpos;
		track_reward_loc = track2_reward_loc;
...
func _process(delta):
	...
	if reward_trial && (head_z>=track_reward_loc) && rewarded==0:
...
#	fpslabel.text = ""
	...
	if (head_z == (track_length/2)-head_radius) || (current_frame > trial_duration*frames_per_second):
		save_logs(track_num,current_rep,yawLog,pitchLog,rollLog,xLog,zLog,angLog,rewardLog,lickLog,timeLog)
		yawLog = [] 
		pitchLog = [] 
		rollLog = [] 
		xLog = [] 
...
func save_logs(track_num,current_rep,col1,col2,col3,col4,col5,col6,col7,col8,col9):
	var file = File.new()
	if (file.open("res://logs//" + timestamp + "_" + scene_name + String(track_num) + "_rep" + String(current_rep) + "_godotlogs.txt", File.WRITE)== OK):
