...
export var force_first_trial = true
export var forced_first_trial_order := [0, 3, 1, 2] #[moveL, 1=darkL, 2=moveR, 3=darkR]
export var max_movement_before_loom = 0.1 #maximum average movement value before a loom will start
export var slow_duration_before_loom = 1 #seconds the mouse has to walk slowly before a loom will start
...
func _ready():	
	...
	if (current_condition>=(num_trials/2)):
		trial_type = current_condition - (num_trials/2)
		direction = -1
	else:
...
func _process(delta):
	...
	if loom_started==false: #if it hasn't already started
		if (current_frame>=(predelay*frames_per_second)) && (movement_avg<=max_movement_before_loom):
			loom_started = true
			loom_ongoing = true
	...
	if loom_started && loom_ongoing:
		...
		#update sky color (control loom trials)
		else:
			if object_zpos<head_z:
				loom_cur_distance = loom_start_distance + loom_step
			...
			if (loom_cur_distance <= loom_stop_distance):
				...
				if (loom_cur_distance <= loom_stop_distance):
					loom_cur_distance = loom_stop_distance
				va = atan2(0.5,loom_cur_distance)
				objectColor = 0.783*(1-((va*va)/(0.5094)))
				object_xpos = head_x + (direction*loom_stop_distance*xscale)
				object_ypos = head_y + (loom_stop_distance*yscale)
				object_zpos = head_z + (loom_stop_distance*zscale)
			object.translation.x = object_xpos
			object.translation.y = object_ypos
			object.translation.z = object_zpos
			objectMaterial.albedo_color = Color(objectColor,objectColor,objectColor,1)
			objectMaterial.flags_unshaded = true
			objectMesh.material_override = objectMaterial
	...
	trialLog.append(current_condition)
	distLog.append(loom_cur_distance)
	timeLog.append(now)
...
#	fpslabel.text = ""
	...
	#next trial
	if (current_frame > (trial_duration*frames_per_second)) && loom_started && (loom_ongoing==false):
		loom_started = false
		current_frame = 1
		current_trial += 1
