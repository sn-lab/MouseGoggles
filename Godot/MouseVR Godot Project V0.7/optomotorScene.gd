...
#	fpslabel.text = ""
	...
	if (start_predelay==0) && (current_frame > trial_duration*frames_per_second):
		...
		if (current_trial==num_trials):
			save_logs(current_rep,yawLog,pitchLog,rollLog,waveLog,dirLog,timeLog)
			yawLog = [] 
			pitchLog = [] 
			rollLog = [] 
			waveLog = []
...
func save_logs(current_rep,col1,col2,col3,col4,col5,col6):
	var file = File.new()
	if (file.open("res://logs//" + timestamp + "_" + scene_name + "rep" + str(current_rep) + "_godotlogs.txt", File.WRITE)== OK):
