...
#	fpslabel.text = ""
	...
	if (head_z == (track_length/2)-head_radius) || (current_frame > trial_duration*frames_per_second):
		save_logs(mouse_num,track_num,current_rep,reward_trial,yawLog,pitchLog,rollLog,xLog,zLog,angLog,rewardLog,lickLog,timeLog)
		yawLog = [] 
		pitchLog = [] 
		rollLog = [] 
		xLog = [] 
...
func save_logs(mouse_num,track_num,current_rep,reward_trial,col1,col2,col3,col4,col5,col6,col7,col8,col9):
	var file = File.new()
	if (file.open("res://logs//" + timestamp + "_" + scene_name + String(track_num) + "_mouse" + String(mouse_num) + "_rep" + String(current_rep) + "_reward" + String(reward_trial)+ "_godotlogs.txt", File.WRITE)== OK):
