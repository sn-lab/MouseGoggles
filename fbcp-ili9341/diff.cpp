...
void NoDiffChangedRectangle(Span *&head, int yStartShift)
{
  ...
  head->y = 0;
  head->endY = actualDisplayHeight;
  head->size = gpuFrameWidth*actualDisplayHeight;
  head->next = 0;
}
#endif
...
void DiffFramebuffersToSingleChangedRectangle(uint16_t *framebuffer, uint16_t *prevFramebuffer, int yStartShift, Span *&head)
{
  ...
  {
    while(minY < actualDisplayHeight)
    {
      int x = 0;
...
found_top:
  ...
  int maxX = -1;
  int maxY = actualDisplayHeight-1;
...
void DiffFramebuffersToScanlineSpansFastAndCoarse4Wide(uint16_t *framebuffer, uint16_t *prevFramebuffer, bool interlacedDiff, int interlacedFieldParity, int yStartShift, Span *&head)
{
  ...
  uint64_t *scanline = (uint64_t *)(framebuffer + (y+yStartShift)*(gpuFramebufferScanlineStrideBytes>>1));
  uint64_t *prevScanline = (uint64_t *)(prevFramebuffer + (y+yStartShift)*(gpuFramebufferScanlineStrideBytes>>1)); // (same scanline from previous frame, not preceding scanline)

  const int W = gpuFrameWidth>>2;

  Span *span = spans;
  while(y < actualDisplayHeight)
  {
...
void DiffFramebuffersToScanlineSpansExact(uint16_t *framebuffer, uint16_t *prevFramebuffer, bool interlacedDiff, int interlacedFieldParity, int yStartShift, Span *&head)
{
  ...
  uint16_t *scanline = framebuffer + (y+yStartShift)*(gpuFramebufferScanlineStrideBytes>>1);
  uint16_t *prevScanline = prevFramebuffer + (y+yStartShift)*(gpuFramebufferScanlineStrideBytes>>1); // (same scanline from previous frame, not preceding scanline)
